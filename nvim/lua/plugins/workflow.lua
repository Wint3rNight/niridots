  ...
  -- Which-key: pause after <space> and it shows what can follow
  {
    ...
    opts = {
      ...
      spec = {
        ...
        { "<leader>h", group = "http (.http files)" },
        { "gr", group = "lsp" },
        { "gs", group = "surround" },
        -- mini.operators live on free g-prefixes; see plugins/editing.lua for why not the defaults
        { "gX", desc = "Exchange region with next" },
        { "gY", desc = "Replace region with register" },
        { "gZ", desc = "Sort region" },
        { "gA", desc = "Duplicate region below" },
        { "gS", desc = "Split/join under cursor" },
        { "[", group = "previous" },
        { "]", group = "next" },
      },
    },
